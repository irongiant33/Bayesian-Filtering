function estimatedTracks = unscentedKalmanFilterNew(observations,parameters)
    ...
    measurementCovariance = diag([rangeVar;angleVar;velocityVar]); 
    ...
    for k=1:numSteps
        ...
        pointsMeasurements(3,:) = 0.5*(((pointsState(1,:) - sensorPosition(1)).^2+(pointsState(2,:) - sensorPosition(2)).^2).^(-0.5)).*(2*(pointsState(1,:) - sensorPosition(1)).*pointsState(3,:)+2*(pointsState(2,:) - sensorPosition(2)).*pointsState(4,:));

        %calculate statistics of transformed state and prediction state
        meanMeasurements = zeros(3,1);
        for i=1:numPoints
            meanMeasurements(1) = meanMeasurements(1) + weights(i) * pointsMeasurements(1,i);
            meanMeasurements(3) = meanMeasurements(3) + weights(i) * pointsMeasurements(3,i);
        end
        ...
        covPrediction = zeros(3,3);
        diffMeasurements = zeros(3,numPoints);
        for i=1:numPoints
