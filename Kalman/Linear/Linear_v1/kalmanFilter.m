function estimatedTracks = kalmanFilter(observations,parameters)
    ...
    stateTransform = [1 0 0 0;0 1 0 0]; %H matrix
    mVar=parameters.sigmaMeasurementNoise^2;
    dVar=parameters.sigmaDrivingNoise^2;
    measurementCovariance = diag([mVar;mVar]);%R matrix
    ...
    for k=1:numSteps
        ...
        estimatedTracks(:,k+1) = priorMean + kalmanGain * (observations(:,k) - stateTransform*priorMean);
