...
observations(1,1:numDevices:numSteps) = sqrt((trueTracks(1,:) - sensorPosition(1)).^2+(trueTracks(2,:) - sensorPosition(2)).^2) + sigmaMeasurementNoiseRange*randn(1,numSteps/numDevices);
observations(3,1:numDevices:numSteps) = 0.5*(((trueTracks(1,:) - sensorPosition(1)).^2+(trueTracks(2,:) - sensorPosition(2)).^2).^(-0.5)).*(2*(trueTracks(1,:) - sensorPosition(1)).*trueTracks(3,:)+2*(trueTracks(2,:) - sensorPosition(2)).*trueTracks(4,:)) + sigmaMeasurementNoiseVelocity*randn(1,numSteps/numDevices);
observations(2,1:numDevices:numSteps) =  atan2d(trueTracks(1,:) - sensorPosition(1), trueTracks(2,:) - sensorPosition(2)) + sigmaMeasurementNoiseBearing*randn(1,numSteps/numDevices);
