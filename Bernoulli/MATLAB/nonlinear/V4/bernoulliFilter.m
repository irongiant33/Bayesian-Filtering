...
for step = 1:numSteps
    ...
    for device = 1:numDevices
        ...
        for n = 1:(numComponents + numBirthComponents)
            ...
            if(device == 1)
                pointsMeasurements(3,:) = 0.5*(((sigmas(1,:) - sensorPosition(1)).^2+(sigmas(2,:) - sensorPosition(2)).^2).^(-0.5)).*(2*(sigmas(1,:) - sensorPosition(1)).*sigmas(3,:)+2*(sigmas(2,:) - sensorPosition(2)).*sigmas(4,:));
            end
